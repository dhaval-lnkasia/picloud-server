...
class Handler {
	...
	public function postOperationClassify(File $file) {
		if (!$this->documentProperties->isDocumentSupported($file->getName())) {
			\OC::$server->getLogger()->debug("[files_classifier] doc type not supported: " . $file->getName());
			return [];
		}

		try {
			$properties = $this->documentProperties->scanFile($file);
		...
		if ($properties === null || $properties === false) {
			\OC::$server->getLogger()->debug("[files_classifier] no metadata extracted: " . $file->getName());
			return [];
		}
	...
	public function fopenClassify($filePath, $originalPath) {
		...
		$properties = $this->documentProperties->scan($filePath, $originalPath);
		$this->propertiesCache[$originalPath] = ($properties ?? false);
